...
class AbgProcess(enum.Enum):
    NORMAL_ABG = enum.auto()
    METABOLIC_ACIDOSIS = enum.auto()
    METABOLIC_ALKALOSIS = enum.auto()
    RESPIRATORY_ACIDOSIS = enum.auto()
    RESPIRATORY_ALKALOSIS = enum.auto()
    UNDEFINED = enum.auto()
...
def calculate_anion_gap(
    ...
    Args:
        ...
        HCO3act: Serum actual bicarbonate (HCO3(P)), mmol/L.
...
def calculate_anion_gap_delta(AG: float, HCO3act: float) -> str:
    ...
    Args:
        ...
        HCO3act: Actual bicarbonate, mmol/L.
    ...
    if gg < 0.4:
        ...
        return info + "(1 ≤ gg ≤ 2): classic high anion gap acidosis. KULT?"
    else:  # 2 < gg
...
def calculate_osmolarity(cNa: float, cGlu: float) -> float:
...
def simple_hco3(pH: float, pCO2: float) -> float:
...
def calculate_hco3p(pH: float, pCO2: float) -> float:
...
def calculate_hco3pst(pH: float, pCO2: float, ctHb: float, sO2: float) -> float:
    ...
    Args:
        ...
        sO2: Fraction of saturated hemoglobin, fraction.
...
def simple_be(pH: float, HCO3act: float) -> float:
...
def calculate_cbase(pH: float, pCO2: float, ctHb: float = 3) -> float:
...
def calculate_hct(ctHb: float) -> float:
...
def calculate_ctO2(
    pO2: float, sO2: float, FCOHb: float, FMetHb: float, ctHb: float
) -> float:
...
def calculate_pO2_FO2_fraction(pO2: float, FiO2: float = 0.21) -> float:
    ...
    Args:
        pO2: kPa
        FiO2: Fraction of oxygen in dry inspired air, fraction.
...
def abg_approach_stable(pH: float, pCO2: float) -> tuple[str, AbgProcess]:
    ...
    def check_metabolic(pH: float, pCO2: float) -> str:
    ...
    if norm_pH[0] <= pH <= norm_pH[1]:  # pH is normal or compensated
        # Don't calculate expected CO2/pH values because both values are
        ...
        if pCO2 < norm_pCO2[0]:
            ...
            if pH >= 7.41:
                return (
                    ...
                    AbgProcess.RESPIRATORY_ALKALOSIS,
                )
            else:
                return (
                    "Metabolic acidosis, full comp. by CO₂ alkalosis",
                    AbgProcess.METABOLIC_ACIDOSIS,
                )
            ...
            if pH <= 7.39:  # pH almost acidic
                ...
                return (
                    ...
                    AbgProcess.RESPIRATORY_ACIDOSIS,
                )
            else:
                return (
                    "Metabolic alkalosis, full comp. by CO₂ acidosis",
                    AbgProcess.METABOLIC_ALKALOSIS,
                )
        else:
            return "Normal ABG", AbgProcess.NORMAL_ABG
    else:
        # pH decompensation
        if pCO2 < norm_pCO2[0]:  # Low (respiratory alkalosis)
            ...
            if pH < norm_pH[0]:
                ...
                return (
                    "Metabolic alkalosis (check high Na⁺, low Cl⁻ or albumin), partial comp. by CO₂ acidosis",
                    AbgProcess.METABOLIC_ALKALOSIS,
                )
        else:
    ...
    return "Impossible to evaluate", AbgProcess.UNDEFINED


def abg_approach_ryabov(pH: float, pCO2: float) -> str:
